...
void sbus_packet_complete_at9s(){
    ...
    int  selected_mode = 0;
//printf("at9s in\n");
    switch(estop_switch) {//SWE
        case AT9S_TRI_UP://SWE

            selected_mode = RC_mode::OFF;

            // if(normal_jump_flip_switch==AT9S_TRI_UP)    //SWG
            // {    
            //     selected_mode = RC_mode::OFF;
            // }else if (normal_jump_flip_switch==AT9S_TRI_MIDDLE)    //SWG
            // {
        ...
        case AT9S_TRI_MIDDLE://SWE
            ...
            {
                selected_mode = RC_mode::RECOVERY_STAND;
            }else if (normal_jump_flip_switch==AT9S_TRI_DOWN)    //SWG
            {
                selected_mode = RC_mode::RL;
                ...
                rc_control.v_des[0] = data.right_stick_x>0? v_scale * data.right_stick_x: v_scale/2.0 * data.right_stick_x; // v_des[0] = 右x   v_des[1] = 右y   v_des[3] = 0
                ...
                rc_control.omega_des[0] = 0;                                                                                 //omega_des[0] = 0   omega_des[1] = 左x(pitch)      omega_des[2]=左y
                rc_control.omega_des[1] = data.left_stick_x;//0;//pitch

                rc_control.rpy_des[0] =roll_show;                                                                            // rpy_des[0] = 0     rpy_des[1] = 0   rpy_des[2] = 0
        ...
        case AT9S_TRI_DOWN:
            ...
            // if(normal_jump_flip_switch==AT9S_TRI_MIDDLE)    //SWG
            // {
            //     if(QP_Locomotion_switch==AT9S_BOOL_UP)     //SWA
            //     {
            //         //selected_mode = RC_mode::LOCOMOTION;
            //         selected_mode = RC_mode::RL;
            //         // Deadband
