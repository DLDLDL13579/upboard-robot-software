...
#ifdef USE_YESENSE
  if(_yesenseInit)
    yesenseThread = std::thread(&MiniCheetahHardwareBridge::runYesenseImu, this);
#else
  // microstrain
  if(_microstrainInit)
...
//   if (_aiport > 0) {
//     int recv_ai=receive_ai(_aiport);
//     if (recv_ai) {      
//     }
//   }
// }
...
#ifdef USE_YESENSE
void MiniCheetahHardwareBridge::runYesenseImu() {
  while (true) {
    auto acc = yesenseImu.getAcc();
    auto quat = yesenseImu.getQuaternion();
    auto gyro = yesenseImu.getAngleVec();


    _vectorNavData.accelerometer[0] = acc[0];
    _vectorNavData.accelerometer[1] = acc[1];
    _vectorNavData.accelerometer[2] = acc[2];
    _vectorNavData.quat[0] = quat[1];
    _vectorNavData.quat[1] = quat[2];
    _vectorNavData.quat[2] = quat[3];
    _vectorNavData.quat[3] = quat[0];
    _vectorNavData.gyro[0] = gyro[0] * 3.14159 / 180.;
    _vectorNavData.gyro[1] = gyro[1] * 3.14159 / 180.;
    _vectorNavData.gyro[2] = gyro[2] * 3.14159 / 180.;
    // std::cout <<"hardware: "<< _vectorNavData.accelerometer << std::endl
    //   << _vectorNavData.quat << std::endl
    //   << _vectorNavData.gyro << std::endl;
    usleep(1000);
  }
}
#endif
...
#ifdef USE_YESENSE
  _yesenseInit = yesenseImu.init("/dev/ttyACM0", 460800);
   // _yesenseInit = yesenseImu.init("/dev/ttyACM0", 460800);
#else
